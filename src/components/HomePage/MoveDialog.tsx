...
const MoveDialog: React.FC<MoveDialogProps> = ({
  ...
  const targetGaragePositions = useMemo(() => {
    ...
    targetGarage,
    targetGarageId,
    selectedVehicleToMove.garageId,
    selectedVehicleToMove.vehicleIndex,
    handleSelectTargetPosition,
  ]);
    ...
    <div className={styles.moveDialogOverlay}>
      <div className={styles.moveDialog}>
        ...
        <div className={styles.moveDialogContent}>
          {moveStep === 2 && (
            <button className={styles.confirmDialogBack} onClick={handleBackToSelectGarage}>
              返回
            </button>
          )}
          {moveStep === 1 ? (
              ...
              <div className={styles.garageList}>
                {garages.map((garage) => (
                  ...
                  >
                    ...
                    <div className={styles.garageCapacity}>
                      {garage.vehicleList.filter((v) => Object.keys(v).length > 0).length} /{' '}
                      {garage.num}
                    </div>
                  </div>
                ))}
            ...
            <>
              ...
              <p>
                到{' '}
                <span style={{ fontSize: '22px' }}>{targetGarage?.storageName || '未知车库'}</span>
              </p>
              <p>选择目标位置:</p>
              <div className={styles.positionList}>{targetGaragePositions}</div>
            </>
          )}
        </div>
        <div className={styles.moveDialogActions}>
          <button className={styles.confirmDialogCancel} onClick={handleCancelMove}>
            取消
          </button>
        </div>
      </div>
